...
ICudaEngine *
FaceFeatExtracter::createEngine(unsigned int maxBatchSize, const std::string &Weight_path, IBuilder *builder,
                                IBuilderConfig *config, DataType dt)
{
    ...
    std::map<std::string, Weights> weightMap = extract_loadWeights(Weight_path);
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    auto bn0 = extract_addBatchNorm2d(network, weightMap, *conv0->getOutput(0), "bn0", 2e-5);
    ...
    auto bn1 = extract_addBatchNorm2d(network, weightMap, *s4u3->getOutput(0), "bn1", 2e-5);
    IFullyConnectedLayer *fc1 = network->addFullyConnected(*bn1->getOutput(0), 512, weightMap["pre_fc1_weight"],
                                                           weightMap["pre_fc1_bias"]);
    assert(fc1);
    auto bn2 = extract_addBatchNorm2d(network, weightMap, *fc1->getOutput(0), "fc1", 2e-5);
