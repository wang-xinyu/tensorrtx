...
namespace nvinfer1 {
    YoloLayerPlugin::YoloLayerPlugin(int classCount, int numberofpoints, float confthreshkeypoints, int netWidth,
        int netHeight, int maxOut, bool is_segmentation, bool is_pose, bool is_obb, const int* strides,
        int stridesLength) {
    ...
    __global__ void CalDetection(const float* input, float* output, int numElements, int maxoutobject, const int grid_h,
        ...
        if (is_obb)
        {
            ...
            auto cos1 = cos(angle); auto sin1 = sin(angle);
            ...
            auto x = xf * cos1 - yf * sin1; auto y = xf * sin1 + yf * cos1;
