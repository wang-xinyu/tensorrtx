...
static float probiou(const Detection& res1, const Detection& res2, float eps = 1e-7) {
    ...
    a1 = std::get<0>(matrix1); b1 = std::get<1>(matrix1); c1 = std::get<2>(matrix1);
    a2 = std::get<0>(matrix2); b2 = std::get<1>(matrix2); c2 = std::get<2>(matrix2);
...
void draw_bbox_obb(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        ...
        for (auto& obj : res) {
            ...
            if (outside){p2.y = corner_points[0].y - height - 3;} else {p2.y = corner_points[0].y + height + 3;}
            cv::rectangle(img, p1, p2, bgr, -1,cv::LINE_AA);
            cv::putText(img, text, cv::Point(corner_points[0].x, (outside ? corner_points[0].y - 2 : corner_points[0].y + height + 2)) , 
