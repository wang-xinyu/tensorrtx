...
class YoLov8TRT(object):
    ...
    def post_process(self, output, origin_h, origin_w,rotated=False):
        ...
        # Do nms
        boxes = self.non_max_suppression(pred, origin_h, origin_w, conf_thres=CONF_THRESH, nms_thres=IOU_THRESHOLD, rotated=True)
        if rotated:
    ...
    def non_max_suppression(self, prediction, origin_h, origin_w, conf_thres=0.5, nms_thres=0.4,rotated=False):
        ...
        if rotated:
            ...
            keep_boxes = []
            while boxes.shape[0]:
                large_overlap = self.batch_probiou(np.expand_dims(boxes[0, columns_index], 0), boxes[:, columns_index]) > nms_thres
...
if __name__ == "__main__":
    ...
    categories = ["plane", "ship", "storage tank", "baseball diamond", "tennis court", "basketball court", "ground track field", "harbor",
