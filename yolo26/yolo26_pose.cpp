...
void serialize_engine(const std::string& wts_name, std::string& engine_name, float& gd, float& gw, int& max_channels,
    ...
    IHostMemory* serialized_engine =
            buildEngineYolo26Pose(builder, config, DataType::kFLOAT, wts_name, gd, gw, max_channels, type);
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, img_dir, type, gd, gw, max_channels)) {
        ...
        std::cerr << "./yolo26_pose -s [.wts] [.engine] [n/s/m/l/x]  // serialize model to "
                     "plan file"
                  << std::endl;
        std::cerr << "./yolo26_pose -d [.engine] ../images  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
