...
nvinfer1::IHostMemory* buildEngineYolo26Seg(nvinfer1::IBuilder* builder, nvinfer1::IBuilderConfig* config,
                                            nvinfer1::DataType dt, const std::string& wts_path, float& gd, float& gw,
                                            int& max_channels, std::string& type)

{
    ...
    *******************************************************************************************************/
    ...
    /////////////////////////////////////////////////////
    // one2one_cv3: per-scale classification branch, identical to buildEngineYolo26Det's one2one_cv3.
    /////////////////////////////////////////////////////
    ...
    /////////////////////////////////////////////////////
    // one2one_cv4: per-scale mask-coefficient branch (32 raw channels per anchor - not sigmoid'd,
    // decoded later against the proto tensor in postprocess). Same Sequential(Conv,Conv,Conv2d)
    // shape as buildEngineYolo26Obb's one2one_cv4, just 1 -> 32 output channels.
    /////////////////////////////////////////////////////

    const int c4 = std::max(get_width(256, gw, max_channels) / 4, 32);
    ...
    nvinfer1::IElementWiseLayer* conv23_one2one_cv4_0_1 = convBnSiLU(
            network, weightMap, *conv23_one2one_cv4_0_0->getOutput(0), c4, {3, 3}, 1, "model.23.one2one_cv4.0.1", 1);
    nvinfer1::IConvolutionLayer* conv23_one2one_cv4_0_2 = network->addConvolutionNd(
            *conv23_one2one_cv4_0_1->getOutput(0), 32, nvinfer1::DimsHW{1, 1},
            weightMap["model.23.one2one_cv4.0.2.weight"], weightMap["model.23.one2one_cv4.0.2.bias"]);
    conv23_one2one_cv4_0_2->setStrideNd(nvinfer1::DimsHW{1, 1});
    conv23_one2one_cv4_0_2->setPaddingNd(nvinfer1::DimsHW{0, 0});
    conv23_one2one_cv4_0_2->setNbGroups(1);
    nvinfer1::IShuffleLayer* reshape23_mask_0 = network->addShuffle(*conv23_one2one_cv4_0_2->getOutput(0));
    reshape23_mask_0->setReshapeDimensions(nvinfer1::Dims3{kBatchSize, 32, -1});
    ...
    nvinfer1::IElementWiseLayer* conv23_one2one_cv4_1_1 = convBnSiLU(
            network, weightMap, *conv23_one2one_cv4_1_0->getOutput(0), c4, {3, 3}, 1, "model.23.one2one_cv4.1.1", 1);
    nvinfer1::IConvolutionLayer* conv23_one2one_cv4_1_2 = network->addConvolutionNd(
            *conv23_one2one_cv4_1_1->getOutput(0), 32, nvinfer1::DimsHW{1, 1},
            weightMap["model.23.one2one_cv4.1.2.weight"], weightMap["model.23.one2one_cv4.1.2.bias"]);
    conv23_one2one_cv4_1_2->setStrideNd(nvinfer1::DimsHW{1, 1});
    conv23_one2one_cv4_1_2->setPaddingNd(nvinfer1::DimsHW{0, 0});
    conv23_one2one_cv4_1_2->setNbGroups(1);
    nvinfer1::IShuffleLayer* reshape23_mask_1 = network->addShuffle(*conv23_one2one_cv4_1_2->getOutput(0));
    reshape23_mask_1->setReshapeDimensions(nvinfer1::Dims3{kBatchSize, 32, -1});
    ...
    nvinfer1::IElementWiseLayer* conv23_one2one_cv4_2_1 = convBnSiLU(
            network, weightMap, *conv23_one2one_cv4_2_0->getOutput(0), c4, {3, 3}, 1, "model.23.one2one_cv4.2.1", 1);
    nvinfer1::IConvolutionLayer* conv23_one2one_cv4_2_2 = network->addConvolutionNd(
            *conv23_one2one_cv4_2_1->getOutput(0), 32, nvinfer1::DimsHW{1, 1},
            weightMap["model.23.one2one_cv4.2.2.weight"], weightMap["model.23.one2one_cv4.2.2.bias"]);
    conv23_one2one_cv4_2_2->setStrideNd(nvinfer1::DimsHW{1, 1});
    conv23_one2one_cv4_2_2->setPaddingNd(nvinfer1::DimsHW{0, 0});
    conv23_one2one_cv4_2_2->setNbGroups(1);
    nvinfer1::IShuffleLayer* reshape23_mask_2 = network->addShuffle(*conv23_one2one_cv4_2_2->getOutput(0));
    reshape23_mask_2->setReshapeDimensions(nvinfer1::Dims3{kBatchSize, 32, -1});

    /////////////////////////////////////////////////////

    nvinfer1::ITensor* inputTensorsMask[] = {reshape23_mask_0->getOutput(0), reshape23_mask_1->getOutput(0),
                                             reshape23_mask_2->getOutput(0)};
    nvinfer1::IConcatenationLayer* cat23_mask = network->addConcatenation(inputTensorsMask, 3);
    cat23_mask->setAxis(2);

    ///////////////////////////////////////////////////////////
    // final output: [box_xyxy(4), cls(kNumClass), mask_coeffs(32)], transposed to
    // [batch, anchors, channels] - handed to the same NMS-free gather plugin buildEngineYolo26Det
    // uses (is_segmentation instead of is_detection).
    ///////////////////////////////////////////////////////////

    nvinfer1::IConcatenationLayer* cat23_3 =
            network->addConcatenation(std::array<nvinfer1::ITensor*, 3>{mul23_2->getOutput(0), sigmoid23->getOutput(0),
                                                                        cat23_mask->getOutput(0)}
                                              .data(),
                                      3);
    cat23_3->setAxis(1);
    ...
    int stridesLength = strides.size();
    nvinfer1::IPluginV2Layer* yolo = addYoloLayer(network, *transpose->getOutput(0), strides, fm_sizes, stridesLength,
                                                  false, true, false, false, anchor_num);
    assert(yolo);
    ...
    *********************************************  YOLO26 SEG PROTO  ***************************************
    *******************************************************************************************************
    * Proto26 (model.23.proto): fuses P4/P5 into P3 resolution, then the usual Proto tower
    * (cv1 -> upsample(ConvTranspose2d x2) -> cv2 -> cv3) to produce the [32, H/4, W/4] prototype
    * masks. Per-anchor mask coefficients above are dot-producted against these protos in
    * postprocess to get the final per-instance masks.
    *******************************************************************************************************/

    nvinfer1::IElementWiseLayer* proto_feat_refine_0 = convBnSiLU(
            network, weightMap, *block19->getOutput(0), get_width(256, gw, max_channels), {1, 1}, 1,
            "model.23.proto.feat_refine.0", 1);
    nvinfer1::IResizeLayer* proto_up_0 = network->addResize(*proto_feat_refine_0->getOutput(0));
    assert(proto_up_0);
    proto_up_0->setResizeMode(nvinfer1::ResizeMode::kNEAREST);
    proto_up_0->setScales(scale, 4);

    nvinfer1::IElementWiseLayer* proto_sum_0 = network->addElementWise(
            *block16->getOutput(0), *proto_up_0->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);

    nvinfer1::IElementWiseLayer* proto_feat_refine_1 = convBnSiLU(
            network, weightMap, *block22->getOutput(0), get_width(256, gw, max_channels), {1, 1}, 1,
            "model.23.proto.feat_refine.1", 1);
    float scale4[] = {1.0, 1.0, 4.0, 4.0};
    nvinfer1::IResizeLayer* proto_up_1 = network->addResize(*proto_feat_refine_1->getOutput(0));
    assert(proto_up_1);
    proto_up_1->setResizeMode(nvinfer1::ResizeMode::kNEAREST);
    proto_up_1->setScales(scale4, 4);

    nvinfer1::IElementWiseLayer* proto_sum_1 = network->addElementWise(
            *proto_sum_0->getOutput(0), *proto_up_1->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);

    nvinfer1::IElementWiseLayer* proto_fuse =
            convBnSiLU(network, weightMap, *proto_sum_1->getOutput(0), get_width(256, gw, max_channels), {3, 3}, 1,
                       "model.23.proto.feat_fuse", 1);

    nvinfer1::IElementWiseLayer* proto_cv1 = convBnSiLU(network, weightMap, *proto_fuse->getOutput(0),
                                                        get_width(256, gw, max_channels), {3, 3}, 1,
                                                        "model.23.proto.cv1", 1);

    nvinfer1::IDeconvolutionLayer* proto_upsample = network->addDeconvolutionNd(
            *proto_cv1->getOutput(0), get_width(256, gw, max_channels), nvinfer1::DimsHW{2, 2},
            weightMap["model.23.proto.upsample.weight"], weightMap["model.23.proto.upsample.bias"]);
    assert(proto_upsample);
    proto_upsample->setStrideNd(nvinfer1::DimsHW{2, 2});
    proto_upsample->setPaddingNd(nvinfer1::DimsHW{0, 0});
    proto_upsample->setNbGroups(1);

    nvinfer1::IElementWiseLayer* proto_cv2 = convBnSiLU(network, weightMap, *proto_upsample->getOutput(0),
                                                        get_width(256, gw, max_channels), {3, 3}, 1,
                                                        "model.23.proto.cv2", 1);

    nvinfer1::IElementWiseLayer* proto_cv3 =
            convBnSiLU(network, weightMap, *proto_cv2->getOutput(0), 32, {1, 1}, 1, "model.23.proto.cv3", 1);

    proto_cv3->getOutput(0)->setName(kProtoTensorName);
    network->markOutput(*proto_cv3->getOutput(0));
