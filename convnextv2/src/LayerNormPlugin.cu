...
template<>
__device__ inline float epsilon<float>() { return 1e-6f; }

template<>
__device__ inline half epsilon<half>() { return (half)1e-6f; }

// --- Kernel ---
// Supports hidden_size up to 1024 with TPB=256, VPT=4
template <typename T, int VPT>
__global__ void layerNormKernel(const T* __restrict__ input, const T* __restrict__ gamma, const T* __restrict__ beta, T* __restrict__ output, int hidden_size, float eps) {
    ...
    if (threadIdx.x == 0) mean = sum / hidden_size;
    __syncthreads();
...
LayerNormPlugin::LayerNormPlugin(const std::string& name, const void* data, size_t length) 
    ...
    const char* a = d;
    mEpsilon = *reinterpret_cast<const float*>(d); d += sizeof(float);
    mHiddenSize = *reinterpret_cast<const int*>(d); d += sizeof(int);
    assert(d == a + length);
}
...
DataType LayerNormPlugin::getOutputDataType(int32_t index, const DataType* inputTypes, int32_t nbInputs) const noexcept {
    return inputTypes[0];
}
...
int32_t LayerNormPlugin::enqueue(const PluginTensorDesc* inputDesc, const PluginTensorDesc* outputDesc, const void* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept {
    ...
    int total = 1;
    for(int i=0; i<inputDesc[0].dims.nbDims; ++i) total *= inputDesc[0].dims.d[i];
    int rows = total / mHiddenSize;

    if (inputDesc[0].type == DataType::kFLOAT) {
        layerNormKernel<float, 4><<<rows, 256, 0, stream>>>(
            (const float*)inputs[0], (const float*)inputs[1], (const float*)inputs[2], (float*)outputs[0], 
            mHiddenSize, mEpsilon);
    } else {
        layerNormKernel<half, 4><<<rows, 256, 0, stream>>>(
            (const half*)inputs[0], (const half*)inputs[1], (const half*)inputs[2], (half*)outputs[0], 
            mHiddenSize, mEpsilon);
    }
    return 0;
}
...
int32_t LayerNormPlugin::initialize() noexcept { return 0; }
...
void LayerNormPlugin::serialize(void* buffer) const noexcept {
    ...
    *reinterpret_cast<float*>(d) = mEpsilon; d += sizeof(float);
    *reinterpret_cast<int*>(d) = mHiddenSize; d += sizeof(int);
}

void LayerNormPlugin::setPluginNamespace(const char* libNamespace) noexcept { mNamespace = libNamespace; }
