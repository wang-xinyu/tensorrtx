...
IActivationLayer* bottleneck(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int inch, int outch, int stride, std::string lname) {
    ...
    IConvolutionLayer* conv1 = network->addConvolution(input, width, DimsHW{1, 1}, weightMap[lname + "conv1.weight"], emptywts);
    assert(conv1);
    ...
    IConvolutionLayer* conv2 = network->addConvolution(*relu1->getOutput(0), width, DimsHW{3, 3}, weightMap[lname + "conv2.weight"], emptywts);
    assert(conv2);
...
int main(int argc, char** argv)
{
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("resnext50.engine");
        if (!p)
        {
