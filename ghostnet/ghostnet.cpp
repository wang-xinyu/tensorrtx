...
ICudaEngine* createEngine(IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("ghostnet.weights");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
...
int main(int argc, char** argv) {
    if (argc != 2) {
        ...
        std::cerr << "./ghost -s   // serialize model to plan file" << std::endl;
        std::cerr << "./ghost -d   // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    if (std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("ghost.engine", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("ghost.engine", std::ios::binary);
        if (file.good()) {
