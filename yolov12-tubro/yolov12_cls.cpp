...
void batch_preprocess(std::vector<cv::Mat> &imgs, float *output,
                      int dst_width = 224, int dst_height = 224) {
  for (size_t b = 0; b < imgs.size(); b++) {
    ...
    cv::resize(img, img, cv::Size(dst_width, dst_height), 0, 0,
               cv::INTER_LINEAR);
    ...
    for (int c = 0; c < 3; ++c) {
      ...
      for (int row = 0; row < dst_height; ++row) {
        for (int col = 0; col < dst_width; ++col) {
          output[b * 3 * dst_height * dst_width + c * dst_height * dst_width +
                 i] = channels[c].at<float>(row, col);
          ++i;
        }
      }
    }
...
std::vector<std::string> read_classes(std::string file_name) {
  ...
  if (!ifs.is_open()) {
    std::cerr << file_name
              << " is not found, pls refer to README and download it."
              << std::endl;
    assert(0);
  }
  std::string s;
...
bool parse_args(int argc, char **argv, std::string &wts, std::string &engine,
                float &gd, float &gw, std::string &img_dir, std::string &type,
                int &max_channels) {
  if (argc < 4)
    return false;
...
void prepare_buffers(ICudaEngine *engine, float **gpu_input_buffer,
                     float **gpu_output_buffer, float **cpu_input_buffer,
  ...
  CUDA_CHECK(
      cudaMalloc((void **)gpu_input_buffer,
...
void infer(IExecutionContext &context, cudaStream_t &stream, void **buffers,
  ...
  CUDA_CHECK(
      cudaMemcpyAsync(buffers[0], input,
                      batchSize * 3 * kClsInputH * kClsInputW * sizeof(float),
...
void serialize_engine(float &gd, float &gw, std::string &wts_name,
                      std::string &engine_name, std::string &type,
                      int max_channels) {
  // Create builder
  ...
  serialized_engine = buildEngineYolov12Cls(
      builder, config, DataType::kFLOAT, wts_name, gd, gw, type, max_channels);
...
int main(int argc, char **argv) {
  ...
  if (!parse_args(argc, argv, wts_name, engine_name, gd, gw, img_dir, type,
                  max_channels)) {
    std::cerr << "arguments not right!" << std::endl;
    std::cerr << "./yolov12-cls -s [.wts] [.engine] [n/s/m/l/x]  // serialize "
                 "model to plan file"
              << std::endl;
    std::cerr << "./yolov12-cls -d [.engine] ../images  // deserialize plan "
  ...
  prepare_buffers(engine, &device_buffers[0], &device_buffers[1],
                  &cpu_input_buffer, &output_buffer_host);
  ...
  for (size_t i = 0; i < file_names.size(); i += kBatchSize) {
    ...
    infer(*context, stream, (void **)device_buffers, cpu_input_buffer,
          output_buffer_host, kBatchSize);
