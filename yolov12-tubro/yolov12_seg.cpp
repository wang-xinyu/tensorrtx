...
const int kOutputSize = kMaxNumOutputBbox *
                            (sizeof(Detection) - sizeof(float) * 51) /
                            sizeof(float) +
                        1;
...
void serialize_engine(std::string &wts_name, std::string &engine_name,
                      std::string &type, float &gd, float &gw,
                      int &max_channels) {
  IBuilder *builder = createInferBuilder(gLogger);
  IBuilderConfig *config = builder->createBuilderConfig();
  IHostMemory *serialized_engine = nullptr;

  serialized_engine = buildEngineYolov12Seg(
      builder, config, DataType::kFLOAT, wts_name, gd, gw, max_channels, type);
...
void prepare_buffer(ICudaEngine *engine, float **input_buffer_device,
                    ...
                    float **output_seg_buffer_device,
                    float **output_buffer_host, float **output_seg_buffer_host,
  ...
  CUDA_CHECK(cudaMalloc((void **)output_seg_buffer_device,
...
void infer(IExecutionContext &context, cudaStream_t &stream, void **buffers,
           float *output, float *output_seg, int batchsize,
  ...
  if (cuda_post_process == "c") {
    ...
    CUDA_CHECK(cudaMemcpyAsync(output_seg, buffers[2],
                               batchsize * kOutputSegSize * sizeof(float),
                               cudaMemcpyDeviceToHost, stream));
    ...
    cuda_decode((float *)buffers[1], model_bboxes, kConfThresh,
                decode_ptr_device, kMaxNumOutputBbox, stream);
...
bool parse_args(int argc, char **argv, std::string &wts, std::string &engine,
                std::string &img_dir, std::string &type,
                std::string &cuda_post_process, std::string &labels_filename,
...
int main(int argc, char **argv) {
  ...
  if (!parse_args(argc, argv, wts_name, engine_name, img_dir, type,
                  cuda_post_process, labels_filename, gd, gw, max_channels)) {
    std::cerr << "Arguments not right!" << std::endl;
    std::cerr << "./yolov12-seg -s [.wts] [.engine] [n/s/m/l/x]  // serialize "
                 "model to plan file"
              << std::endl;
    std::cerr << "./yolov12-seg -d [.engine] ../images   coco_file// "
  ...
  prepare_buffer(engine, &device_buffers[0], &device_buffers[1],
                 &device_buffers[2], &output_buffer_host,
                 &output_seg_buffer_host, &decode_ptr_host, &decode_ptr_device,
                 cuda_post_process);
  ...
  for (size_t i = 0; i < file_names.size(); i += kBatchSize) {
    ...
    // Preprocess
    cuda_batch_preprocess(img_batch, device_buffers[0], kInputW, kInputH,
                          stream);
    // Run inference
    infer(*context, stream, (void **)device_buffers, output_buffer_host,
          output_seg_buffer_host, kBatchSize, decode_ptr_host,
    ...
    if (cuda_post_process == "c") {
      ...
      for (size_t b = 0; b < img_batch.size(); b++) {
        ...
        auto masks = process_mask(&output_seg_buffer_host[b * kOutputSegSize],
                                  kOutputSegSize, res);
      ...
      // batch_process(res_batch, decode_ptr_host, img_batch.size(),
      // bbox_element, img_batch); todo seg in gpu
