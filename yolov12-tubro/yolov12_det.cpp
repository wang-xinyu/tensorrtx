...
void serialize_engine(std::string &wts_name, std::string &engine_name,
  ...
  serialized_engine = buildEngineYolov12Det(
...
int main(int argc, char **argv) {
  ...
  if (!parse_args(argc, argv, wts_name, engine_name, img_dir, type,
                  cuda_post_process, gd, gw, max_channels)) {
    std::cerr << "Arguments not right!" << std::endl;
    std::cerr << "./yolov12-det -s [.wts] [.engine] [n/s/m/l/x]  // serialize "
                 "model to "
                 "plan file"
              << std::endl;
    std::cerr << "./yolov12-det -d [.engine] ../images  [c/g]  // deserialize "
  ...
  for (size_t i = 0; i < file_names.size(); i += kBatchSize) {
    ...
    infer(*context, stream, (void **)device_buffers, output_buffer_host,
          kBatchSize, decode_ptr_host, decode_ptr_device, model_bboxes,
          cuda_post_process);
