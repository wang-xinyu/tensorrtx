...
static __global__ void decode_kernel_obb(float *predict, int num_bboxes,
...
static __device__ void convariance_matrix(float w, float h, float r, float &a,
                                          float &b, float &c) {
...
static __device__ float box_probiou(float cx1, float cy1, float w1, float h1,
                                    float r1, float cx2, float cy2, float w2,
...
static __global__ void nms_kernel_obb(float *bboxes, int max_objects,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (pitem[4] >= pcurrent[4]) {
      if (pitem[4] == pcurrent[4] && i < position)
        continue;
      float iou = box_probiou(pcurrent[0], pcurrent[1], pcurrent[2],
                              pcurrent[3], pcurrent[7], pitem[0], pitem[1],
...
void cuda_decode_obb(float *predict, int num_bboxes, float confidence_threshold,
  ...
  int block = 256;
  int grid = ceil(num_bboxes / (float)block);
  decode_kernel_obb<<<grid, block, 0, stream>>>(
      (float *)predict, num_bboxes, confidence_threshold, parray, max_objects);
}
...
void cuda_nms_obb(float *parray, float nms_threshold, int max_objects,
  ...
  int block = max_objects < 256 ? max_objects : 256;
  int grid = ceil(max_objects / (float)block);
  nms_kernel_obb<<<grid, block, 0, stream>>>(parray, max_objects,
                                             nms_threshold);
}
