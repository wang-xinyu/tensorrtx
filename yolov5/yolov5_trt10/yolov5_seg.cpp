...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, float& gd, float& gw,
                std::string& img_dir, std::string& labels_filename) {
    if (argc < 4)
        return false;
...
void prepare_buffers(ICudaEngine* engine, float** gpu_input_buffer, float** gpu_output_buffer1,
    ...
    TensorIOMode output_proto_mode = engine->getTensorIOMode(kProtoTensorName);
    if (output_proto_mode != TensorIOMode::kOUTPUT) {
        std::cerr << kOutputTensorName << " should be output tensor" << std::endl;
        assert(false);
    }
...
void infer(IExecutionContext& context, cudaStream_t& stream, void** buffers, float* output1, float* output2,
           int batchSize) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(output1, buffers[1], batchSize * kOutputSize1 * sizeof(float), cudaMemcpyDeviceToHost,
                               stream));
    CUDA_CHECK(cudaMemcpyAsync(output2, buffers[2], batchSize * kOutputSize2 * sizeof(float), cudaMemcpyDeviceToHost,
                               stream));
...
void serialize_engine(unsigned int max_batchsize, float& gd, float& gw, std::string& wts_name,
    ...
    serialized_engine = build_seg_engine(max_batchsize, builder, config, DataType::kFLOAT, gd, gw, wts_name);
...
int main(int argc, char** argv) {
    // -s ../models/yolov5n-seg.wts ../models/yolov5n-seg.fp32.trt n
    // -d ../models/yolov5n-seg.fp32.trt ../images coco.txt
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, gd, gw, img_dir, labels_filename)) {
        ...
        std::cerr << "./yolov5_seg -s [.wts] [.engine] [n/s/m/l/x or c gd gw]  // serialize model to plan file"
                  << std::endl;
        std::cerr << "./yolov5_seg -d [.engine] ../images coco.txt  // deserialize plan file, read the labels file and "
                     "run inference"
                  << std::endl;
        return -1;
    }
