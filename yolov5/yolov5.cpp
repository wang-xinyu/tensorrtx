...
ICudaEngine* createEngine_s(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("E:\\LearningCodes\\tensorrtx\\yolov5V2\\yolov5V2\\weights\\yolov5sV2_1.wts");
...
ICudaEngine* createEngine_m(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
	...
	std::map<std::string, Weights> weightMap = loadWeights("E:\\LearningCodes\\tensorrtx\\yolov5V2\\yolov5V2\\weights\\yolov5mV2.wts");
	Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
...
ICudaEngine* createEngine_l(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
	...
	std::map<std::string, Weights> weightMap = loadWeights("E:\\LearningCodes\\tensorrtx\\yolov5V2\\yolov5V2\\weights\\yolov5lV2.wts");
	Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
...
ICudaEngine* createEngine_x(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
	...
	std::map<std::string, Weights> weightMap = loadWeights("E:\\LearningCodes\\tensorrtx\\yolov5V2\\yolov5V2\\weights\\yolov5xV2.wts");
	Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("yolov5sv2.engine", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("yolov5sv2.engine", std::ios::binary);
        if (file.good()) {
		...
		for (int b = 0; b < fcount; b++) {
			...
			for (int row = 0; row < INPUT_H; ++row) {
				...
				for (int col = 0; col < INPUT_W; ++col) {
					data[b * 3 * INPUT_H * INPUT_W + i] = uc_pixel[2] / 255;
					data[b * 3 * INPUT_H * INPUT_W + i + INPUT_H * INPUT_W] = uc_pixel[1] / 255.0;
					data[b * 3 * INPUT_H * INPUT_W + i + 2 * INPUT_H * INPUT_W] = uc_pixel[0] / 255.0;
					uc_pixel += 3;
					++i;
				}
			}
