...
if __name__ == "__main__":
    ...
    engine_file_path = "build/yolo11n-pose.engine"
    ...
    try:
        ...
        # Warm up
        for i in range(10):
            _, _ = yolo11_wrapper.infer(yolo11_wrapper.get_raw_image_zeros())
        ...
        # variables for computing and showing FPS
        frame_count = 0
        fps = 0
        ...
        while True:
            ...
            # inference
            processed_frames, inference_time = yolo11_wrapper.infer(frame_generator())
            ...
            fps_text = f"FPS: {fps:.2f} | Inference: {inference_time*1000:.2f}ms"
            cv2.putText(output_frame, fps_text, fps_pos, font, font_scale, font_color, font_thickness, cv2.LINE_AA)

            # shows the prediction
            cv2.imshow('YOLO11 TensorRT Inference', output_frame)
