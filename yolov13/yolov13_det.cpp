...
void prepare_buffer(ICudaEngine* engine, IExecutionContext& context, float** input_buffer_device,
                    float** output_buffer_device, float** output_buffer_host, float** decode_ptr_host,
                    float** decode_ptr_device, std::string cuda_post_process, int& output_size) {
    assert(engine->getNbIOTensors() == 2);
    // In order to bind the buffers, we need to know the names of the input and output tensors.
    // Note that indices are guaranteed to be less than IEngine::getNbBindings()

    // Set input shape for TensorRT 10 (Required for enqueueV3 if engine has dynamic shapes)
    if (!context.setInputShape(kInputTensorName, Dims4(kBatchSize, 3, kInputH, kInputW))) {
        std::cerr << "Error: Could not set input shape for " << kInputTensorName << std::endl;
        exit(1);
    }

    // Get output tensor shape to determine size dynamically
    auto out_dims = context.getTensorShape(kOutputTensorName);
    output_size = 1;
    ...
    CUDA_CHECK(cudaMalloc((void**)output_buffer_device, kBatchSize * output_size * sizeof(float)));

    if (!context.setTensorAddress(kInputTensorName, *input_buffer_device)) {
        std::cerr << "Error: Tensor '" << kInputTensorName << "' not found in engine. Check config.h" << std::endl;
        exit(1);
    }
    if (!context.setTensorAddress(kOutputTensorName, *output_buffer_device)) {
        std::cerr << "Error: Tensor '" << kOutputTensorName << "' not found in engine. Check config.h" << std::endl;
        exit(1);
    }

    if (cuda_post_process == "c") {
        *output_buffer_host = new float[kBatchSize * output_size];
    } else if (cuda_post_process == "g") {
        if (kBatchSize > 1) {
            std::cerr << "Do not yet support GPU post processing for multiple batches" << std::endl;
            exit(0);
        }
        size_t decode_buffer_size = kBatchSize * (1 + kMaxNumOutputBbox * bbox_element) * sizeof(float);
        // Allocate memory for decode_ptr_host and copy to device
        *decode_ptr_host = new float[kBatchSize * (1 + kMaxNumOutputBbox * bbox_element)];
        memset(*decode_ptr_host, 0, decode_buffer_size);
...
void infer(IExecutionContext& context, cudaStream_t& stream, void** buffers, float* output, int batchsize,
    ...
    if (!context.enqueueV3(stream)) {
        std::cerr << "Error: TensorRT inference (enqueueV3) failed!" << std::endl;
    }
    if (cuda_post_process == "c") {
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, img_dir, type, cuda_post_process, gd, gw, max_channels)) {
        ...
        std::cerr << "./yolov13_det -s [.wts] [.engine] [n/s//l/x]  // serialize model to "
                     "plan file"
                  << std::endl;
        std::cerr << "./yolov13_det -d [.engine] ../images  [c/g]// deserialize plan file and run inference"
                  << std::endl;
        return -1;
    }
    ...
    model_bboxes = kMaxNumOutputBbox;
    ...
    prepare_buffer(engine, *context, &device_buffers[0], &device_buffers[1], &output_buffer_host, &decode_ptr_host,
                   &decode_ptr_device, cuda_post_process, output_size);
    ...
    for (size_t i = 0; i < file_names.size(); i += kBatchSize) {
        ...
        for (size_t j = i; j < i + kBatchSize && j < file_names.size(); j++) {
            ...
            if (img.empty()) {
                std::cerr << "Error: Could not read image " << file_names[j] << ". Skipping." << std::endl;
                continue;
            }
        ...
        infer(*context, stream, (void**)device_buffers, output_buffer_host, kBatchSize, decode_ptr_host,
              decode_ptr_device, model_bboxes, cuda_post_process, output_size);
        // Save the first 100 values of output_buffer_host, one per line
        std::vector<std::vector<Detection>> res_batch;
        if (cuda_post_process == "c") {
            // NMS
            batch_nms(res_batch, output_buffer_host, img_batch.size(), output_size, kConfThresh, kNmsThresh);
        } else if (cuda_post_process == "g") {
        ...
        const std::string out_dir = get_executable_dir();
#if defined(_WIN32)
