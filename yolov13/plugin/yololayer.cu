...
size_t YoloLayerPlugin::getSerializationSize() const TRT_NOEXCEPT {
    return sizeof(mClassCount) + sizeof(mThreadCount) + sizeof(mNetHeight) + sizeof(mNetWidth) + sizeof(mMaxOutObject) +
           sizeof(mStridesLength) + sizeof(int) * mStridesLength;
}
...
nvinfer1::IPluginV2IOExt* YoloLayerPlugin::clone() const TRT_NOEXCEPT {
    YoloLayerPlugin* p =
            new YoloLayerPlugin(mClassCount, mNetWidth, mNetHeight, mMaxOutObject, mStrides, mStridesLength);
...
int YoloLayerPlugin::enqueue(int batchSize, const void* TRT_CONST_ENQUEUE* inputs, void* const* outputs,
    ...
    forwardGpu((const float* const*)inputs, (float*)outputs[0], stream, mNetHeight, mNetWidth, batchSize);
    return 0;
}
...
void YoloLayerPlugin::forwardGpu(const float* const* inputs, float* output, cudaStream_t stream, int mNetHeight,
                                 int mNetWidth, int batchSize) {
    ...
    for (int i = 0; i < maxGrids; ++i) {
        flatGrids[2 * i] = mNetHeight / mStrides[i];
        flatGrids[2 * i + 1] = mNetWidth / mStrides[i];
    }
