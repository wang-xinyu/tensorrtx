...
void cuda_preprocess(uint8_t* src, int src_width, int src_height, float* dst, int dst_width, int dst_height,
    ...
    if (img_size > g_max_image_size * 3) {
        std::cerr << "Error: Image size (" << src_width << "x" << src_height
                  << ") exceeds kMaxInputImageSize. Increase it in config.h" << std::endl;
        return;
    }
...
void cuda_preprocess_init(int max_image_size) {
    g_max_image_size = max_image_size;
