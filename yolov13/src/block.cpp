...
nvinfer1::ILayer* FuseModule(nvinfer1::INetworkDefinition* network, std::map<std::string, nvinfer1::Weights> weightMap,
    ...
    float scale[] = {1, 1, 2, 2};
    x3_up->setResizeMode(nvinfer1::InterpolationMode::kNEAREST);
...
nvinfer1::ISoftMaxLayer* AdaHyperedgeGen(nvinfer1::INetworkDefinition* network,
    ...
    nvinfer1::Weights context_net_wts = weightMap[lname + ".context_net.weight"];
    nvinfer1::Weights context_net_bias = weightMap[lname + ".context_net.bias"];
    nvinfer1::IConstantLayer* context_net_wts_layer =
            network->addConstant(nvinfer1::Dims2{node_dim * 2, num_hyperedges * node_dim}, context_net_wts);
    nvinfer1::IConstantLayer* context_net_bias_layer =
            network->addConstant(nvinfer1::Dims2{1, num_hyperedges * node_dim}, context_net_bias);
    nvinfer1::IMatrixMultiplyLayer* context_cat_mul =
            network->addMatrixMultiply(*context_cat, nvinfer1::MatrixOperation::kNONE,
                                       *context_net_wts_layer->getOutput(0), nvinfer1::MatrixOperation::kNONE);
    nvinfer1::IElementWiseLayer* prototype_offsets =
            network->addElementWise(*context_cat_mul->getOutput(0), *context_net_bias_layer->getOutput(0),
                                    nvinfer1::ElementWiseOperation::kSUM);
    nvinfer1::IShuffleLayer* prototype_offsets_dim3 = network->addShuffle(*prototype_offsets->getOutput(0));
    prototype_offsets_dim3->setReshapeDimensions(nvinfer1::Dims3{B, num_hyperedges, D});
    ...
    nvinfer1::IElementWiseLayer* prototypes = network->addElementWise(
            *prototype_base->getOutput(0), *prototype_offsets_dim3->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
    // prototypes = self.prototype_base.unsqueeze(0) + prototype_offsets

    nvinfer1::Weights X_proj_wts = weightMap[lname + ".pre_head_proj.weight"];
    nvinfer1::Weights X_proj_bias = weightMap[lname + ".pre_head_proj.bias"];
    nvinfer1::IConstantLayer* X_proj_wts_ = network->addConstant(nvinfer1::Dims3{1, node_dim, node_dim}, X_proj_wts);
    nvinfer1::IConstantLayer* X_proj_bias_ = network->addConstant(nvinfer1::Dims3{1, 1, node_dim}, X_proj_bias);
    nvinfer1::IMatrixMultiplyLayer* X_proj = network->addMatrixMultiply(
            input, nvinfer1::MatrixOperation::kNONE, *X_proj_wts_->getOutput(0), nvinfer1::MatrixOperation::kNONE);
    nvinfer1::IElementWiseLayer* X_proj_result = network->addElementWise(
            *X_proj->getOutput(0), *X_proj_bias_->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
    // X_proj = self.pre_head_proj(X)

    nvinfer1::IShuffleLayer* X_heads = network->addShuffle(*X_proj_result->getOutput(0));
    ...
    nvinfer1::IShuffleLayer* pro_heads = network->addShuffle(*prototypes->getOutput(0));
    pro_heads->setReshapeDimensions(nvinfer1::Dims4{B, num_hyperedges, num_heads, head_dim});
    pro_heads->setSecondTranspose(nvinfer1::Permutation{0, 2, 1, 3});
    ...
    nvinfer1::IShuffleLayer* proto_heads_flat = network->addShuffle(*pro_heads->getOutput(0));
...
nvinfer1::IElementWiseLayer* AdaHGConv(nvinfer1::INetworkDefinition* network,
    ...
    nvinfer1::Weights edge_proj_wts = weightMap[lname + ".edge_proj.0.weight"];
    nvinfer1::Weights edge_proj_bias = weightMap[lname + ".edge_proj.0.bias"];
    nvinfer1::IConstantLayer* edge_proj_wts_layer =
            network->addConstant(nvinfer1::Dims3{1, embed_dim, embed_dim}, edge_proj_wts);
    nvinfer1::IConstantLayer* edge_proj_bias_layer =
            network->addConstant(nvinfer1::Dims3{1, 1, embed_dim}, edge_proj_bias);
    nvinfer1::IMatrixMultiplyLayer* edge_proj_mul =
            network->addMatrixMultiply(*He->getOutput(0), nvinfer1::MatrixOperation::kNONE,
                                       *edge_proj_wts_layer->getOutput(0), nvinfer1::MatrixOperation::kNONE);
    nvinfer1::IElementWiseLayer* edge_proj_result = network->addElementWise(
            *edge_proj_mul->getOutput(0), *edge_proj_bias_layer->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);
    nvinfer1::IActivationLayer* edge_proj_result_gelu =
            network->addActivation(*edge_proj_result->getOutput(0), nvinfer1::ActivationType::kGELU_ERF);
    // He = self.edge_proj(He)

    nvinfer1::IMatrixMultiplyLayer* X_new =
            network->addMatrixMultiply(*A->getOutput(0), nvinfer1::MatrixOperation::kNONE,
                                       *edge_proj_result_gelu->getOutput(0), nvinfer1::MatrixOperation::kNONE);
    // X_new = torch.bmm(A, He)

    nvinfer1::Weights node_proj_wts = weightMap[lname + ".node_proj.0.weight"];
    nvinfer1::Weights node_proj_bias = weightMap[lname + ".node_proj.0.bias"];
    nvinfer1::IConstantLayer* node_proj_wts_layer =
            network->addConstant(nvinfer1::Dims3{1, embed_dim, embed_dim}, node_proj_wts);
    nvinfer1::IConstantLayer* node_proj_bias_layer =
            network->addConstant(nvinfer1::Dims3{1, 1, embed_dim}, node_proj_bias);
    nvinfer1::IMatrixMultiplyLayer* node_proj_mul =
            network->addMatrixMultiply(*X_new->getOutput(0), nvinfer1::MatrixOperation::kNONE,
                                       *node_proj_wts_layer->getOutput(0), nvinfer1::MatrixOperation::kNONE);
    nvinfer1::IElementWiseLayer* node_proj_result = network->addElementWise(
            *node_proj_mul->getOutput(0), *node_proj_bias_layer->getOutput(0), nvinfer1::ElementWiseOperation::kSUM);

    nvinfer1::IActivationLayer* node_proj_result_gelu =
            network->addActivation(*node_proj_result->getOutput(0), nvinfer1::ActivationType::kGELU_ERF);
    // X_new = self.node_proj(X_new)

    nvinfer1::IElementWiseLayer* X_new_add_input =
            network->addElementWise(*node_proj_result_gelu->getOutput(0), input, nvinfer1::ElementWiseOperation::kSUM);

    return X_new_add_input;
}
