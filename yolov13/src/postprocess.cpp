...
cv::Rect get_rect(cv::Mat &img, float bbox[4]) {
  ...
  t = std::max(0.0f, t);
  int width =
      std::max(0, std::min(int(round(r - l)), img.cols - int(round(l))));
  int height =
      std::max(0, std::min(int(round(b - t)), img.rows - int(round(t))));
...
cv::Rect get_rect_adapt_landmark(cv::Mat &img, float bbox[4],
                                 float lmk[kNumberOfPoints * 3]) {
  float l, r, t, b;
  float r_w = kInputW / (img.cols * 1.0);
  float r_h = kInputH / (img.rows * 1.0);
  if (r_h > r_w) {
...
void process_decode_ptr_host(std::vector<Detection> &res,
                             const float *decode_ptr_host, int bbox_element,
...
void batch_process(std::vector<std::vector<Detection>> &res_batch,
                   const float *decode_ptr_host, int batch_size,
  ...
  for (int i = 0; i < batch_size; i++) {
    ...
    process_decode_ptr_host(res_batch[i], &decode_ptr_host[i * count],
                            bbox_element, img, count);
  }
}
...
void draw_bbox_keypoints_line(std::vector<cv::Mat> &img_batch,
                              std::vector<std::vector<Detection>> &res_batch) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      for (int k = 0; k < kNumberOfPoints * 3; k += 3) {
        if (res[j].keypoints[k + 2] > 0.5) {
          cv::circle(
              img,
              cv::Point((int)res[j].keypoints[k], (int)res[j].keypoints[k + 1]),
              3, cv::Scalar(0, 0x27, 0xC1), -1);
        }
      }
      ...
      for (const auto &bone : skeleton_pairs) {
        ...
        if (res[j].keypoints[kp1_idx + 2] > 0.5 &&
            res[j].keypoints[kp2_idx + 2] > 0.5) {
          cv::Point p1((int)res[j].keypoints[kp1_idx],
                       (int)res[j].keypoints[kp1_idx + 1]);
          cv::Point p2((int)res[j].keypoints[kp2_idx],
                       (int)res[j].keypoints[kp2_idx + 1]);
...
void process_decode_ptr_host_obb(std::vector<Detection> &res,
                                 const float *decode_ptr_host, int bbox_element,
                                 cv::Mat &img, int count) {
  Detection det;
  for (int i = 0; i < count; i++) {
...
void batch_process_obb(std::vector<std::vector<Detection>> &res_batch,
                       const float *decode_ptr_host, int batch_size,
                       int bbox_element,
  ...
  for (int i = 0; i < batch_size; i++) {
    auto &img = const_cast<cv::Mat &>(img_batch[i]);
    process_decode_ptr_host_obb(res_batch[i], &decode_ptr_host[i * count],
                                bbox_element, img, count);
  }
}
...
static float probiou(const Detection &res1, const Detection &res2,
                     float eps = 1e-7) {
  ...
  float t1 =
      ((a1 + a2) * std::pow(y1 - y2, 2) + (b1 + b2) * std::pow(x1 - x2, 2)) /
      ((a1 + a2) * (b1 + b2) - std::pow(c1 + c2, 2) + eps);
  float t2 = ((c1 + c2) * (x2 - x1) * (y1 - y2)) /
             ((a1 + a2) * (b1 + b2) - std::pow(c1 + c2, 2) + eps);
  float t3 = std::log(((a1 + a2) * (b1 + b2) - std::pow(c1 + c2, 2)) /
                          (4 * std::sqrt(std::max(a1 * b1 - c1 * c1, 0.0f)) *
                               std::sqrt(std::max(a2 * b2 - c2 * c2, 0.0f)) +
                           eps) +
                      eps);
...
void nms_obb(std::vector<Detection> &res, float *output, float conf_thresh,
             float nms_thresh) {
  int det_size = sizeof(Detection) / sizeof(float);
  std::map<float, std::vector<Detection>> m;
...
void batch_nms_obb(std::vector<std::vector<Detection>> &res_batch,
...
static std::vector<cv::Point> get_corner(cv::Mat &img, const Detection &box) {
  ...
  corner_points[0] =
      cv::Point(int(round(x_ + vec1x + vec2x)),
                int(round(y_ + vec1y + vec2y))); // Top-left corner
  corner_points[1] =
      cv::Point(int(round(x_ + vec1x - vec2x)),
                int(round(y_ + vec1y - vec2y))); // Top-right corner
  corner_points[2] =
      cv::Point(int(round(x_ - vec1x - vec2x)),
                int(round(y_ - vec1y - vec2y))); // Bottom-right corner
  corner_points[3] =
      cv::Point(int(round(x_ - vec1x + vec2x)),
                int(round(y_ - vec1y + vec2y))); // Bottom-left corner

  // Check and adjust corner points to ensure the rectangle is parallel to image
  // boundaries
...
void draw_bbox_obb(std::vector<cv::Mat> &img_batch,
                   std::vector<std::vector<Detection>> &res_batch) {
  static std::vector<uint32_t> colors = {
      0xFF3838, 0xFF9D97, 0xFF701F, 0xFFB21D, 0xCFD231, 0x48F90A, 0x92CC17,
      0x3DDB86, 0x1A9334, 0x00D4BB, 0x2C99A8, 0x00C2FF, 0x344593, 0x6473FF,
  ...
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (auto &obj : res) {
      ...
      cv::polylines(img, std::vector<std::vector<cv::Point>>{corner_points},
                    true, bgr, 1);

      auto text = (std::to_string((int)(obj.class_id)) + ":" +
                   to_string_with_precision(obj.conf));
      ...
      cv::putText(img, text,
                  cv::Point(corner_points[0].x,
                            (outside ? corner_points[0].y - 2
                                     : corner_points[0].y + height + 2)),
