...
def cuda_memcpy_htod_async(ptr, nparray, stream):
    err, = cuda.cuMemcpyHtoDAsync(ptr, nparray.ctypes.data, nparray.nbytes, stream)
    check_cuda_err(err)


def cuda_memcpy_dtoh_async(nparray, ptr, stream):
    err, = cuda.cuMemcpyDtoHAsync(nparray.ctypes.data, ptr, nparray.nbytes, stream)
    check_cuda_err(err)
...
class YoLov13TRT(object):
    ...
    def infer(self, raw_image_generator):
        ...
        batch_origin_h = []
        batch_origin_w = []
        batch_input_image = np.zeros(shape=[self.batch_size, 3, self.input_h, self.input_w], dtype=np.float32)
