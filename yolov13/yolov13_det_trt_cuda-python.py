...
def cuda_memcpy_htod_async(ptr, nparray, stream):
    err, = cuda.cuMemcpyHtoDAsync(
        ptr, nparray.ctypes.data, nparray.nbytes, stream)
    check_cuda_err(err)


def cuda_memcpy_dtoh_async(nparray, ptr, stream):
    err, = cuda.cuMemcpyDtoHAsync(
        nparray.ctypes.data, ptr, nparray.nbytes, stream)
    check_cuda_err(err)


def pagelocked_empty(shape, dtype):
    bytes_count = np.prod(shape) * np.dtype(dtype).itemsize
    err, ptr = cudart.cudaMallocHost(bytes_count)
    check_cuda_err(err)
    # Create a writable buffer from the pointer
    buffer = (ctypes.c_char * bytes_count).from_address(ptr)
...
class YoLov13TRT(object):
    ...
    def __init__(self, engine_file_path):
        ...
        check_cuda_err(err)
        err, self.ctx = cuda.cuCtxCreate(None, 0, dev)
        check_cuda_err(err)
        ...
        for i in range(engine.num_io_tensors):
            ...
            print('tensor:', tensor_name, engine.get_tensor_shape(tensor_name))
            self.batch_size = engine.get_tensor_shape(tensor_name)[0]
            print(f"The engine s batch-size is {self.batch_size}", )
            dtype = trt.nptype(engine.get_tensor_dtype(tensor_name))
            # Allocate host and device buffers
            size = trt.volume(engine.get_tensor_shape(tensor_name))
            host_mem = pagelocked_empty(size, dtype)
            cuda_mem = cuda_mem_alloc(host_mem.nbytes)
            # Append the device buffer to device bindings.
            bindings.append(int(cuda_mem))
            # Append to the appropriate list.
            if engine.get_tensor_mode(tensor_name) == trt.TensorIOMode.INPUT:
                self.input_w = engine.get_tensor_shape(tensor_name)[-1]
                self.input_h = engine.get_tensor_shape(tensor_name)[-2]
    ...
    def infer(self, raw_image_generator):
        ...
        err, = cuda.cuCtxPushCurrent(self.ctx)
        check_cuda_err(err)
        ...
        batch_origin_h = []
        batch_origin_w = []
        batch_input_image = np.zeros(
            shape=[self.batch_size, 3, self.input_h, self.input_w], dtype=np.float32)
        ...
        cuda_memcpy_htod_async(cuda_inputs[0], host_inputs[0], stream)
        # Run inference.
        context.execute_async_v3(stream_handle=stream)
        # Transfer predictions back from the GPU.
        cuda_memcpy_dtoh_async(host_outputs[0], cuda_outputs[0], stream)
        ...
        # Do postprocess
        for i in range(len(batch_origin_h)):
            result_boxes, result_scores, result_classid = self.post_process(
                output[i * self.det_output_length: (i + 1) *
                       self.det_output_length], batch_origin_h[i],
                batch_origin_w[i]
            )
    ...
    def destroy(self):
        ...
        err, ctx = cuda.cuCtxPopCurrent()
        check_cuda_err(err)
        # Explicitly delete TRT objects before destroying the CUDA context
        del self.context
        del self.engine
...
class warmUpThread(threading.Thread):
    ...
    def run(self):
        batch_image_raw, use_time = self.yolov13_wrapper.infer(
            self.yolov13_wrapper.get_raw_image_zeros())
        print(
...
if __name__ == "__main__":
    ...
    try:
        ...
        image_path_batches = get_img_path_batches(
            yolov13_wrapper.batch_size, image_dir)
