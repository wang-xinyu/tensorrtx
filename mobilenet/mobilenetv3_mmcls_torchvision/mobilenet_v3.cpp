...
int makeDivisible(int value, int divisor, int minValue) {
    int newValue = std::max(minValue, int((float(value) + float(divisor) / 2)) / divisor * divisor);
    if(float(newValue) < 0.9 * float(value)) {
        newValue += divisor;
    }
    return newValue;
}
...
ILayer* seLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int c, int w, std::string lname) {
    ...
    IFullyConnectedLayer* l2 = network->addFullyConnected(*l1->getOutput(0), makeDivisible(BS*c/4, 8, 0), weightMap[lname+"fc.0.weight"], weightMap[lname+"fc.0.bias"]);
...
ILayer* convSeq1(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int output, int hdim, int k, int s, bool use_se, bool use_hs, int w, std::string lname) {
    ...
    IScaleLayer* bn2 = addBatchNorm(network, weightMap, *conv2->getOutput(0), lname + "5", 1e-3);
    assert(bn2);
    return bn2;
}
...
ILayer* convSeq2(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int output, int hdim, int k, int s, bool use_se, bool use_hs, int w, std::string lname) {
    ...
    IScaleLayer* bn2 = addBatchNorm(network, weightMap, *conv2->getOutput(0), lname + "4", 1e-3);
    if (use_hs) {
        ILayer* hsw2 = hSwish(network, *bn2->getOutput(0), lname + "5");
    ...
    if (use_se) {
         ILayer* se1 = seLayer(network, weightMap, *tensor6, hdim, w, lname + "6.");
    ...
    IScaleLayer* bn3 = addBatchNorm(network, weightMap, *conv3->getOutput(0), lname + "8", 1e-3);
    assert(bn3);
    return bn3;
}
...
ICudaEngine* createEngineSmall(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    IFullyConnectedLayer* fc2 = network->addFullyConnected(*sw2->getOutput(0), OUTPUT_SIZE, weightMap["classifier.2.weight"], weightMap["classifier.2.bias"]);
...
ICudaEngine* createEngineLarge(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    auto ir14 = invertedRes(network, weightMap, *ir13->getOutput(0), "features.14.", 160, 160, 2, 960, 5, 1, 1, 7);
