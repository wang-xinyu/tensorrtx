...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../mobilenetv2.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    // flatten pool1 layer.
    IShuffleLayer* pool1FlattenLayer = network->addShuffle(*pool1->getOutput(0));
    pool1FlattenLayer->setReshapeDimensions(Dims2{1, 1280});

    ITensor* pool1FlattenLayerOutput = pool1FlattenLayer->getOutput(0);

    // reshape fc weight.
    Dims fcWeightDims = Dims2{1000, 1280};
    Weights fcW = weightMap["classifier.1.weight"];
    ...
    IMatrixMultiplyLayer* fcMatrixMultiplyLayer =
            network->addMatrixMultiply(*pool1FlattenLayerOutput, MatrixOperation::kNONE, *fcWeightLayer->getOutput(0),
                                       MatrixOperation::kTRANSPOSE);
    assert(fcMatrixMultiplyLayer);

    // add fc bias
    Dims fcBiasDims = Dims2{1, 1000};
    Weights fcBias = weightMap["classifier.1.bias"];
